...
For each post, I distribute it to the appropriate platforms via the native platform APIs (postToMeta, postToLinkedIn, postToX, postToTikTok, postToPinterest), then update `posted_platforms` on the draft. Posts are spaced at least 3 hours apart per platform to respect rate limits.
